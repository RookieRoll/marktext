...
- [x] 6.1 在大型项目夹具上运行初始加载测试，验证 `loadMarkdownFile` 初始调用为 0、IPC 发送次数远小于节点数、树提交次数有界且最终树与文件系统一致
...
已完成 3.2、3.3：扫描期事件按 pathname 合并为 Map，同一路径只保留最后一次状态，add 会清除此前记录的删除，replay 顺序确定且有界；新增“同一路径反复 change 只重放一次”“删除后重建不丢文件”用例。

已完成 3.5、3.6、3.7：快照早于 OPEN_PROJECT 时按 root 暂存并在打开时应用；切换 root 会整体替换树并清空 pending；新增 pending 事件在快照后重放、跨项目隔离用例。
...
已完成 5.1-5.4：创建/重命名通知由树容器单点订阅，行通过 focusRegistry 按 row key 注册处理函数；上下文菜单改为容器 `data-pathname` 事件委托，行组件不再持有 DOM 监听；行组件不再使用 storeToRefs。
