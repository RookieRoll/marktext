...
// The platform facades resolve preload capabilities at call time. Reuse the
// module instance and reset only the globals each test depends on. Resetting
// the whole module graph for every test makes the first dynamic import exceed
// Vitest's default timeout and can let a timed-out test leak deleted globals.
const loadPdf = async () => ({ getCssForOptions, getHtmlToc })

const resetPdfGlobals = () => {
